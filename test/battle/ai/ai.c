...
AI_SINGLE_BATTLE_TEST("AI won't Sucker Punch if outsped by priority")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY);
        PLAYER(SPECIES_RAICHU_ALOLA) { Moves(MOVE_PSYCHIC, MOVE_THUNDERBOLT, MOVE_QUICK_ATTACK, MOVE_ELECTROWEB); }
        OPPONENT(SPECIES_ZIGZAGOON) { Level(1); Moves(MOVE_SCRATCH); }
        OPPONENT(SPECIES_CACTURNE) { Moves(MOVE_ENERGY_BALL, MOVE_DARK_PULSE, MOVE_SCORCHING_SANDS, MOVE_SUCKER_PUNCH); }
    } WHEN {
        TURN { MOVE(player, MOVE_QUICK_ATTACK); EXPECT_MOVE(opponent, MOVE_SCRATCH); EXPECT_SEND_OUT(opponent, ); }
        TURN { MOVE(player, MOVE_QUICK_ATTACK); EXPECT_MOVE(opponent, MOVE_DARK_PULSE); }
    }
}
